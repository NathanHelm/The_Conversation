...
public class CharacterManager : StaticInstance<CharacterManager>, ISaveLoad, IExecution
{
    ...
    private Dictionary<int, CharacterNoDialogueObject> characterIDToCharacterNoDialogue = new();
    ...
    public void SetUpCharacterIDtoConversationIdToConversation() //method adds all occuring characterMono and their lines to the dictionary
    {
        ...
        for (int i = 0; i < characterMono.Length; i++)
        {
            ...
            if (characterMono[i].interviewFaceStreamingAssetsImageName == null)
            {
                Debug.LogError("character " + bodyId + " has no interview face texture");
            }
            AddCharacterToDict(new CharacterNoDialogueObject(characterMono[i].persistentConversationQuestionId, characterMono[i].interviewFaceStreamingAssetsImageName), bodyId);
    ...
    public void AddCharacterToDict(CharacterNoDialogueObject characterNoDialogueObjects, int bodyId)
    {
        if(characterIDToCharacterNoDialogue.ContainsKey(bodyId))
        {
            Debug.LogError("duplicate character id's "+ bodyId + "in scene.");
            return;
        }

        characterIDToCharacterNoDialogue.Add(bodyId, characterNoDialogueObjects);
