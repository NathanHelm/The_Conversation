Shader "Unlit/EdgeDetectionGame"
{
    Properties
    {
        ...
        _EdgeDetectionTex ("Texture", 2D) = "white" {}
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
               ...
               // float2 wave = float2(sin(i.uv.y * _WiggleFrequency), cos(i.uv.x * _WiggleFrequency)) * _WiggleAmplitude;
                float3 renderOffset = tex2D(_EdgeDetectionTex, i.uv);
                renderOffset = float3(0 , 0 , 0);
                float3 sobelNormalVec = SobelSample(_CameraDepthNormalsTexture, i.uv + renderOffset, offset, 1).rgb;

                float sobelNormal = sobelNormalVec.x + sobelNormalVec.y + sobelNormalVec.z;
                sobelNormal = pow(sobelNormal * _OutlineNormalMultiplier, _OutlineNormalBias);
                float sobelDepth = SobelSampleDepth(i.uv + renderOffset, offset, _NoiseStrength);
