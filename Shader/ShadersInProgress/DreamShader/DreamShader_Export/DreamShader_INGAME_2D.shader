Shader "Unlit/DreamShader_INGAME_2D"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass 
        {
            ...
            float3 TransitionTexandDreamTex(float3 a, float3 b)
            {
                /*
                
                */
                _angle = _Time.x;
                _sinValue = sin(_angle * _speed);

                if(_sinValue < 0)
                {
                   _speed = 0.5 * _SinTime; //slow time
                }
                else
                {
                    _speed = 2 * _SinTime;
                ...
                float _val = _sinValue;

                return lerp(a, b, _val);
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                col = TransitionTexandDreamTex(tex2D(_MainTex, i.uv).rgb, col);
                return float4(col, 1.);
            }
            ENDCG
