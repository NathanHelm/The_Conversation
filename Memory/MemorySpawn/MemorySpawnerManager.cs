...
public class MemorySpawnerManager : StaticInstance<MemorySpawnerManager>, ISaveLoad, IExecution, IObserver<ObserverAction.MemoryTransform>
{
    ...
    public List<MemoryStageCreatedObject> spawnedMemoryStageObject { get; set; } = new();
    ...
    private readonly Vector3 subMemory3dOffset = new Vector3(200, 0, 0);
    ...
    public void AddSpawnedStageObject(MemorySpawnObject memoryObject, GameObject createdMemoryObject)
    {
        spawnedMemoryStageObject.Add(new(createdMemoryObject, memoryObject));
    }

    public void AddSpawnedSubStageObject(GameObject subObject2d, GameObject subObject3d)
    {
        spawnedMemoryStageObject[^1].spawnedSubMemory.Add(new(subObject2d, subObject3d));
    }
    ...
    public void SpawnStageOnID(int characterID, int memoryID)
    {
        ...
        AddSpawnedStageObject(memoryStageObject, CreateMemoryStage(memoryStageObject.memoryGameObject));
    }
    ...
    public void SpawnSubStageOnID(int characterID, int memoryID, int subMemoryId, GameObject parent)
    {
        ...
        subMemoryStageGameObject2D.AddComponent<PolygonCollider2D>();
        ...
        AddSpawnedSubStageObject(subMemoryStageGameObject2D, subMemoryStageGameObject3D);
    ...
    public void SetTransformStagePos()
    {
        ...
        foreach (var single in spawnedMemoryStageObject)
        {
            foreach (var subMemories in single.spawnedSubMemory)
            {
                //2d object
                PhysicsGameObject2d physicsGameObject2D = subMemories.subStage2d.GetComponent<PhysicsGameObject2d>();
                //3d object

                TwoTo3dPositions twoTo3DPositions = subMemories.subStage3d.GetComponent<TwoTo3dPositions>();

                physicsGameObject2D.SetInitialLocalPos(); //reset so that inital 2d position account from transform stage
                twoTo3DPositions.physicsGameObject2D = physicsGameObject2D; //making sure 2d position aligns with 3d position. 
                Vector3 offset = physicsGameObject2D.transform.localPosition + subMemory3dOffset;
    ...
    public void Load()
    {
        ...
        jsonUnlockMemoryObjects.ForEach(character =>
        {
            foreach (var memoryObject in character.memoryObjects)
            {
                ...
                if (Data.MemoryData.CURRENTCHARACTERID == character.characterID)
                {
                    ...
                    foreach (var subMemoryObject in memoryObject.subMemoryIds)
                    {
                        ...
                        var stageObj = spawnedMemoryStageObject[^1].spawnedMemoryStage; //getting the memoryobject object.
    ...
    {
        if (data == MemoryTransform.onAfterEnableTransformation)
        {
